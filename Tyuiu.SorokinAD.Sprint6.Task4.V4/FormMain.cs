...
using Tyuiu.SorokinAD.Sprint6.Task4.V4.Lib;
using System.IO;
    ...
    public partial class FormMain : Form
    {
        ...
        private void buttonResult_SAD_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                chartFunction_SAD.Series[0].Points.Clear();
                for (int i = 0; i <= len - 1; i++)
                {
                    this.chartFunction_SAD.Series[0].Points.AddXY(startStep, valueArray[i]);
                    textBoxOutPut_SAD.AppendText(valueArray[i] + Environment.NewLine);
                    startStep++;
                }
            }
        ...
        {
            MessageBox.Show("Таск 4 выполнил студент группы ИИПБ-23-2 Сорокин Артём Дмитриевич", "Сообщение", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }

        private void buttonSave_SAD_Click(object sender, EventArgs e)
        {
            try
            {
                string path = $@"{Directory.GetCurrentDirectory()}/OutPutFileTask4.txt";
                File.WriteAllText(path, textBoxOutPut_SAD.Text);
        ...
        private void buttonSave_SAD_MouseClick(object sender, MouseEventArgs e)
        {

        }

        private void groupBoxTask_SAD_Enter(object sender, EventArgs e)
        {
